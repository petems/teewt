    ...
    class Config < Base
      def call(env)
        ...
        if env['user_hide_keys']
          ...
          config_data = config_data.gsub(%r{(consumer_key: )([a-zA-Z0-9]+)}, '\1 [REDACTED]')
          config_data = config_data.gsub(%r{(consumer_secret: )([a-zA-Z0-9]+)}, '\1 [REDACTED]')
          config_data = config_data.gsub(%r{(access_token: )([a-zA-Z0-9-]+)}, '\1 [REDACTED]')
          config_data = config_data.gsub(%r{(access_token_secret: )([a-zA-Z0-9]+)}, '\1 [REDACTED]')
        end
